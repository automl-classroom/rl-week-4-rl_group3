...
class ReplayBuffer(AbstractBuffer):
    ...
    def add(
        ...
        if len(self.states) >= self.capacity:
            # pop the oldest element off each list (states, actions, …, infos)
            self.states.pop(0)
            self.actions.pop(0)
            self.rewards.pop(0)
    ...
    def sample(
        ...
        """
        # randomly choose `batch_size` unique indices from [0, len(self.states))
        idxs = np.random.choice(a=self.__len__(), size=batch_size, replace=False)
        return [
            (
                self.states[i],
                self.actions[i],
